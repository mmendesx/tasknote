...
// Feature modules (empty stubs — implementations land in ICT-5..14)
...
@Module({
  imports: [
    // Load env vars from process.env — no .env file required, just env vars
    ConfigModule.forRoot({ isGlobal: true }),

    TypeOrmModule.forRootAsync({
      useFactory: (): BetterSqlite3ConnectionOptions => {
        // Resolve and guarantee the DB directory exists — explicit rather than
        // relying on a side-effect from importing health.controller or data-source.
        ...
        return {
          ...
          migrationsRun: true,
          synchronize: false,
          // WAL mode and foreign keys enabled before any statement
